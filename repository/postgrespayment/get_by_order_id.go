...
func (d *DB) GetByOrderID(ctx context.Context, orderID uint64) (*paymententity.Payment, error) {
	...
	query := `
		SELECT id, order_id, user_id, method, amount, currency, status, external_id, provider_reference_id, payment_url, idempotency_key,CallbackData , ExpiredAt, created_at, updated_at
