...
func (s *Service) ProcessManualWalletRecharge(ctx context.Context, req checkoutparams.ManualRechargeRequest) error {
	...
	if err != nil {
		...
		logger.Logger.Error("checkout manual recharge failed",
			zap.Error(err),
			zap.Uint64("user_id", req.UserID),
			zap.String("idempotency_key", idempotencyKey[:16]+"..."),
	...
	fmt.Println(req.UserID, req.AdminID)
	_ = s.messenger.Send(ctx, &bot.SendMessageParams{
		ChatID: req.UserTelegramID,
		Text:   fmt.Sprintf("Your wallet charged: %d by admin", req.Amount),
	})

	_ = s.messenger.Send(ctx, &bot.SendMessageParams{
		ChatID: req.AdminID,
		Text:   fmt.Sprintf("Admin %d recharged user %d: %d", req.AdminID, req.UserID, req.Amount),
	})
